    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {mailItems.filter(item => !item.is_read).length > 0 && (
          <div className="px-4 py-2 bg-blue-100 dark:bg-blue-500/10 border border-blue-300 dark:border-blue-500/30 rounded-lg">
      ...
      {/* Info Banner */}
      <div className="p-4 bg-gray-100 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-xl">
        <div className="flex items-start gap-3">
          <Package className="w-5 h-5 text-emerald-500 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
          <div className="text-sm">
            <p className="text-gray-700 dark:text-slate-300 font-medium mb-1">What mail do I receive?</p>
      ...
      {error && (
        <div className="p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-xl flex items-start gap-3">
          ...
          <div>
            <p className="text-red-600 dark:text-red-400 font-medium">Error loading mailbox</p>
            <p className="text-sm text-red-500/80 dark:text-red-300/70 mt-1">{error}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-4">
        {mailItems.length === 0 ? (
          <div className="text-center py-16 bg-gray-50 dark:bg-slate-900/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-slate-800">
            <Mail className="w-16 h-16 text-gray-400 dark:text-slate-600 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-gray-600 dark:text-slate-400 mb-2">No mail received yet</h3>
            ...
            .map((item) => (
              <div
                ...
                className={`
                  ...
                  ${item.is_read 
                    ? 'bg-gray-50 dark:bg-slate-900/30 border-gray-200 dark:border-slate-800' 
                    : 'bg-white dark:bg-slate-800/50 border-l-4 border-l-blue-500 border-gray-200 dark:border-slate-700'
                  }
                `}
              >
                  ...
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <h4 className={`font-semibold ${item.is_read ? 'text-gray-700 dark:text-slate-300' : 'text-gray-900 dark:text-white'}`}>
